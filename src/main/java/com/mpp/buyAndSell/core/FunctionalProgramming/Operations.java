...
public class Operations {
	...
	public Function<List<User>, List<User>> blockedUsers = (l) -> l.stream()
			.filter(user -> user.isBlocked().equals(true)).collect(Collectors.toList());

	// used in item service
	public Function<List<Item>, ItemCategoryEnum> topCategory = (Items) -> Items.stream()
			.collect(Collectors.groupingBy(Item::getCategory)).entrySet().stream()
	...
	public BiFunction<List<Comment>, User, List<Comment>> userComments = (c, u) -> c.stream()
			.filter(x -> x.getUser().equals(u)).collect(Collectors.toList());

	// used in user service
	public BiFunction<List<Item>, User, List<Item>> userItems = (I, U) -> I.stream().filter(x -> x.getUser().equals(U))
	...
	public Function<List<Item>, List<User>> Top5Sellers = (I) -> I.stream()
			...
			.sorted((i1, i2) -> i2.getValue().size() - i1.getValue().size()).limit(5).map(list -> list.getKey())
			.collect(Collectors.toList());
	...
	public Function<List<Item>, Double> totalItemPriceInQuarter = l -> l.stream()
			...
			.map(item -> item.getPrice()).reduce((p1, p2) -> p1 + p2).get();
